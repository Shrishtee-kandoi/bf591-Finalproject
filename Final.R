...
ui <- fluidPage(theme = shinythemes::shinytheme("cerulean"),
  ...
  tabsetPanel(
    ...
    tabPanel("DE",
             ...
             p("To use this application, upload a CSV file containing the differential expression data"),
             sidebarLayout(
               sidebarPanel(
    ...
    tabPanel("IGE",
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 submitButton(text='Plot', width = '100%')
               ),
               mainPanel(
                 plotOutput("IGE_plot")
               )
...
server <- function(input, output, session){
  ...
  output$normID <- renderTable({
    counts_summ(counts_load(), input$varID, input$nonzero)
  })
  ...
  output$plotTable <- renderDataTable({
    draw_table(de_load(), input$padjmag)
  })
  output$IGE_plot <- renderPlot({
    plot_distro(load_4counts(), IGE_meta(), input$metachoice, input$gene)
  })
}
